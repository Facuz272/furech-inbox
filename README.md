...
**Otras:** `messages.created_at` y `conversations.last_message_at` son `timestamptz(3)`: `Date` de JS tiene precisión de milisegundos y `timestamptz` de microsegundos; probando encontré que el cursor (que viaja como ISO string) podía saltear una fila que cayera en esos microsegundos, y que un `Date` devuelto a Postgres perdía precisión. Alinear la columna al borde JS lo resuelve de raíz. `last_message_at` es siempre `max(created_at)` de sus mensajes (mismo `UPDATE` para entrante y saliente), no "cuándo llegó": así coincide con el `lastMessage` embebido. `pg` con SQL crudo en vez de ORM, coherente con "schema a mano" y sin capa que explicar. Una conversación por contacto (`UNIQUE (contact_id)`) — sin estados open/closed en este alcance. El mensaje saliente se guarda `pending` y se commitea **antes** de llamar al proveedor, para no sostener una conexión abierta esperando un servicio externo y para que un fallo quede como `failed` reintentable.
